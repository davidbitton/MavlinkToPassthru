...
Average *average = new Average(bat1, bat2);
...
#elif defined(ARDUINO_BLUEPILL_F103C6)   // Blue Pill
    Debug.println("Blue Pill STM32F103C");
#elif defined(ARDUINO_MAPLE_MINI_ORIGIN) //  Maple Mini
...
void DecodeOneMavFrame() {
    ...
    if (!MavRingBuff.isEmpty()) {
        ...
        case MAVLINK_MSG_ID_SYS_STATUS: // #1
            if (!mavGood)
                break;
            ap_voltage_battery1 = average->Get_Volt_Average1(mavlink_msg_sys_status_get_voltage_battery(&msg));    // 1000 = 1V  i.e mV
            ap_current_battery1 = average->Get_Current_Average1(mavlink_msg_sys_status_get_current_battery(&msg)); //  100 = 1A, i.e dA
        ...
        case MAVLINK_MSG_ID_BATTERY2: // #181   http://mavlink.org/messages/ardupilotmega
            if (!mavGood)
                break;
            ap_voltage_battery2 = average->Get_Volt_Average2(mavlink_msg_battery2_get_voltage(&msg));            // 1000 = 1V
            ap_current_battery2 = average->Get_Current_Average2(mavlink_msg_battery2_get_current_battery(&msg)); //  100 = 1A
...
void MarkHome() {
    ...
    Debug.print("hom.lat=");
    Debug.print(hom.lat, 7);
    Debug.print(" hom.lon=");
    Debug.print(hom.lon, 7);
    Debug.print(" hom.alt=");
    Debug.print(hom.alt, 1);
    Debug.print(" hom.hdg=");
    Debug.println(hom.hdg);
#endif
}
//***************************************************
...
*/
  ...
  */
    ...
    for (int i = 2; i < lth; i++) {
        DisplayByte(bytes[i]);
        if (i == 7)
            Debug.print("  "); // Print space after header
    }
    Debug.println();
}
//***************************************************
